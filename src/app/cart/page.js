...
export default function Cart() {
  ...
  const placeOrder = () => {
    ...
    const orderSubtotal = productDetails.reduce(
    ...
    axios
      .post(
        "https://node-razor-pay.onrender.com/api/frontend/orders/place-order",
        datasava
      )
      .then((success) => {
        console.log(success)
  ...
  const openPaymentPopUp = (order_id, razorpayOrder) => {
    ...
    rzp1.on("payment.failed", function (response) {
      ...
      axios
        .post(
          "https://node-razor-pay.onrender.com/api/frontend/orders/confirm-order",
          {
            order_id: response.razorpay_order_id,
            payment_id: response.razorpay_payment_id,
            status: 3,
    ...
    <div className="container mx-auto p-4 md:p-10">
      <div className="flex flex-col md:flex-row shadow-md my-5">
        ...
        <div className="w-full md:w-3/4 bg-white p-5 md:p-10">
          ...
          {cart.length > 0 ? (
            cart.map((item, index) => (
              <div
                key={index}
                className="flex flex-col md:flex-row items-center md:items-start border-b py-4 md:py-5"
              >
                ...
                <div className=" w-full md:w-1/5 flex flex-col items-center mt-2 md:mt-0">
                  <span className="text-gray-600 text-xs uppercase mb-1">
                    Qty
                  </span>
                ...
                <div className="w-full md:w-1/5 text-center font-semibold text-sm mt-2 md:mt-0">
                  ...
                  <span className="ml-1 text-black block  ">
                ...
                <div className="w-full flex justify-center md:w-1/5 text-center font-semibold text-sm mt-2 md:mt-0">
                  <button
                    className="table cursor-pointer  bg-[red] p-1 text-white "
                    onClick={() => deleteRow(index)}
                  >
                    Delete
                  </button>
          ...
          <a
            href="#"
            className="text-indigo-600 text-sm mt-5 block text-center md:text-left"
          >
            Continue Shopping
          </a>
        </div>
        ...
        <div className="w-full md:w-1/4 p-5 md:p-10 bg-gray-100 mt-5 md:mt-0">
          <h1 className="font-semibold text-lg md:text-2xl border-b pb-4">
            Order Summary
          </h1>
          ...
          <div className="mt-4">
            ...
            />
            <button className="bg-red-500 w-full text-white py-2 mt-2 rounded-md">
              Apply
            </button>
          </div>
          <div className="border-t mt-6 pt-4">
            ...
            <button
              className="bg-indigo-500 text-white w-full py-3 mt-4 rounded-md cursor-pointer"
              onClick={placeOrder}
            >
              Checkout
            </button>
